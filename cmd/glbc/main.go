...
import (
	...
	"k8s.io/ingress-gce/pkg/readonly"
...
func main() {
	...
	realKubeClient, err := kubernetes.NewForConfig(kubeConfigForProtobuf)
	if err != nil {
	...
	realEventRecorderKubeClient, err := kubernetes.NewForConfig(restclient.AddUserAgent(kubeConfigForProtobuf, "l7lb-events"))
	if err != nil {
	...
	realCloud := app.NewGCEClient(rootLogger)

	var kubeClient kubernetes.Interface = realKubeClient
	var eventRecorderKubeClient kubernetes.Interface = realEventRecorderKubeClient
	var cloud *gce.Cloud = realCloud

	if flags.F.ReadOnlyMode {
		rootLogger.Info("CONTROLLER IS RUNNING IN READ-ONLY MODE. NO CHANGES WILL BE MADE.")

		// Wrap the real clients with our read-only decorators
		kubeClient = readonly.NewReadOnlyKubeClient(realKubeClient, rootLogger)
		// Note: We might also want a NoOp Event Recorder to avoid flooding logs
		eventRecorderKubeClient = readonly.NewReadOnlyKubeClient(realEventRecorderKubeClient, rootLogger)
		vals := gce.DefaultTestClusterValues()
		cloud = gce.NewFakeGCECloud(vals)
	}
