...
func TestGetNodePrimaryIP(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			desc: "IPv4 InternalIP",
			node: &api_v1.Node{
				Status: api_v1.NodeStatus{
					Addresses: []api_v1.NodeAddress{
						{Type: api_v1.NodeInternalIP, Address: "1.2.3.4"},
					},
				},
			},
			wantIP: "1.2.3.4",
		...
		{
			desc: "Dual stack IPv6 first prefers IPv4",
			node: &api_v1.Node{
				Status: api_v1.NodeStatus{
					Addresses: []api_v1.NodeAddress{
						{Type: api_v1.NodeInternalIP, Address: "2001:db8::1"},
		...
		{
			desc: "IPv6 only falls back to IPv6",
			node: &api_v1.Node{
				Status: api_v1.NodeStatus{
					Addresses: []api_v1.NodeAddress{
						{Type: api_v1.NodeInternalIP, Address: "2001:db8::1"},
		...
		},
		{
			...
			node: &api_v1.Node{
				Status: api_v1.NodeStatus{
					Addresses: []api_v1.NodeAddress{
						{Type: api_v1.NodeExternalIP, Address: "11.12.13.14"},
					},
				},
			},
			wantIP: "",
	...
	for _, tc := range testCases {
		...
		t.Run(tc.desc, func(t *testing.T) {
			t.Parallel()
			got := utils.GetNodePrimaryIP(tc.node, klog.TODO())
			if got != tc.wantIP {
				t.Errorf("GetNodePrimaryIP() = %q, want %q", got, tc.wantIP)
			}
		})
	}
}
