...
func (l4 *L4) provideDualStackHealthChecks(nodeNames []string, result *L4ILBSyncResult) string {
	...
	if hcResult.HCFirewallRuleName != "" {
		...
		metaapi.RemoveStatusCondition(&result.Conditions, l4conditions.FirewallHealthCheckConditionType)
	}
	...
	if hcResult.HCFirewallRuleIPv6Name != "" {
		...
		metaapi.RemoveStatusCondition(&result.Conditions, l4conditions.FirewallHealthCheckIPv6ConditionType)
	}
...
func (l4 *L4) ensureIPv4Resources(result *L4ILBSyncResult, nodeNames []string, options gce.ILBOptions, bs *composite.BackendService, existingFR *composite.ForwardingRule, subnetworkURL, ipToUse string) {
	...
	case forwardingrules.ProtocolTCP:
		...
		metaapi.RemoveStatusCondition(&result.Conditions, l4conditions.UDPForwardingRuleConditionType)
		metaapi.RemoveStatusCondition(&result.Conditions, l4conditions.L3ForwardingRuleConditionType)
	case forwardingrules.ProtocolUDP:
		...
		metaapi.RemoveStatusCondition(&result.Conditions, l4conditions.TCPForwardingRuleConditionType)
		metaapi.RemoveStatusCondition(&result.Conditions, l4conditions.L3ForwardingRuleConditionType)
